...
public class LoginActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        hc=new HttpCalls(getApplicationContext(), "XXX");
        if(!session.isLocLoaded()){
            hc.downloadLocations();
            Log.d(TAG, ""+session.isLocLoaded());
        }
        if(!session.isContLoaded()){
            hc.downloadContacts();
            Log.d(TAG, "" + session.isContLoaded());
        }
            ...
            public void onResponse(String response) {
                ...
                try {
                    ...
                    if (!error) {
                        ...
                        hc=new HttpCalls(getApplicationContext(), email);
                        download();
    ...
    private void download(){
        showDialog();
        if(session.isLoggedIn()){
            if(!session.isSemesterLoaded()){
                hc.downloadSemester();
                Log.d(TAG, "" + session.isSemesterLoaded());
            }
            if(!session.isEventLoaded()){
                hc.downloadEvent();
                Log.d(TAG, "" + session.isEventLoaded());
            }
            if(!session.isCORSESLoaded()){
                hc.downloadClasses();
                Log.d(TAG, "" + session.isCORSESLoaded());
            }
